import TiktokCommentGenerate from "@/components/blocks/tiktok-comment-generate";
...
export async function generateMetadata({
  ...
  const messages = await import(`@/i18n/pages/tiktok-comment-generate/${locale}.json`);
  const section = messages.default.tiktok_comment_generate;
  const metadata = section.metadata;

  let canonicalUrl = `${process.env.NEXT_PUBLIC_WEB_URL}/tiktok-comment-generator`;
  if (locale !== "en") {
    canonicalUrl = `${process.env.NEXT_PUBLIC_WEB_URL}/${locale}/tiktok-comment-generator`;
  }
  ...
  return {
    ...
    alternates: {
      ...
      languages: buildLanguageAlternates("/tiktok-comment-generator"),
    },
    openGraph: {
      title: metadata.title,
      description: metadata.description,
...
export default async function TiktokCommentGeneratorPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  return (
    <>
      ...
      <RelatedTools
        currentSlug="tiktok-comment-generator"
        limit={6}
