import MurderMysteryGenerate from "@/components/blocks/murder-mystery-generate";
...
export async function generateMetadata({
  ...
  let canonicalUrl = `${process.env.NEXT_PUBLIC_WEB_URL}/murder-mystery-generator`;
  if (locale !== "en") {
    canonicalUrl = `${process.env.NEXT_PUBLIC_WEB_URL}/${locale}/murder-mystery-generator`;
  }

  const messages = await import(`@/i18n/pages/murder-mystery-generate/${locale}.json`);
  const section = messages.default.murder_mystery_generate;
  ...
  return {
    ...
    alternates: {
      ...
      languages: buildLanguageAlternates("/murder-mystery-generator"),
    },
    openGraph: {
      title: metadata.title,
      description: metadata.description,
...
export default async function MurderMysteryGeneratorPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const currentUrl = `${homeUrl}${locale === "en" ? "" : `/${locale}`}/murder-mystery-generator`;
  ...
  const webAppSchema = {
    ...
    featureList: [
      "Complete murder mystery scenario generation",
      "Suspects with motives and alibis",
      "Numbered clues and red herrings",
      "Host-only solution section",
      "Scalable player count (4-10+)",
  ...
  return (
    <>
      ...
      <MurderMysteryGenerate section={section} />
      ...
      <ModuleToolsSection
        ...
        excludeSlug="murder-mystery-generator"
        accent="orange"
      />
