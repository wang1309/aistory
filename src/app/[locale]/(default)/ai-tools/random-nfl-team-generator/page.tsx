import RandomNflTeamGenerator from "@/components/blocks/random-nfl-team-generator";
...
export async function generateMetadata({
  ...
  const messages = await import(
    `@/i18n/pages/random-nfl-team-generator/${locale}.json`
  );
  const section = messages.default.random_nfl_team_generator;
  ...
  return {
    ...
    alternates: {
      ...
      languages: buildLanguageAlternates("/ai-tools/random-nfl-team-generator"),
    },
    openGraph: {
      title: metadata.title,
      description: metadata.description,
...
export default async function RandomNflTeamGeneratorPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const currentUrl = `${homeUrl}${locale === "en" ? "" : `/${locale}`}/ai-tools/random-nfl-team-generator`;
    ...
    "@graph": [
      {
        ...
        itemListElement: [
          ...
          },
          {
            "@type": "ListItem",
            position: 3,
            name: section.ui?.breadcrumb_current ?? "Random NFL Team Generator",
            item: currentUrl,
          },
        ],
      },
      {
        "@type": "WebApplication",
        name: section.ui?.title ?? "Random NFL Team Generator",
  ...
  return (
    <>
      ...
      />
      <RandomNflTeamGenerator section={section} />
      {section.feature1 && (
        <FeatureIntro section={section.feature1} accent="orange" />
      )}
      {section.how_to_use && (
        <HowToUse section={section.how_to_use} accent="orange" />
      )}
      {section.feature2 && <Benefits section={section.feature2} accent="orange" />}
      {section.feature3 && (
        <UseCases section={section.feature3} accent="orange" />
      )}
      {section.faq && <FAQ section={section.faq} accent="orange" />}
      <RelatedTools
        currentSlug="random-nfl-team-generator"
        relatedSlugs={[
          "band-name-generator",
          "emoji-translator",
          "gang-name-generator",
