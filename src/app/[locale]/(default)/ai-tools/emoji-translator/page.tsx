import EmojiTranslator from "@/components/blocks/emoji-translator";
...
export async function generateMetadata({
  ...
  const messages = await import(`@/i18n/pages/emoji-translator/${locale}.json`);
  const section = messages.default.emoji_translator;
  const metadata = section.metadata;

  const canonicalUrl =
    locale === "en"
      ? `${process.env.NEXT_PUBLIC_WEB_URL}/ai-tools/emoji-translator`
      : `${process.env.NEXT_PUBLIC_WEB_URL}/${locale}/ai-tools/emoji-translator`;
  ...
  return {
    ...
    alternates: {
      ...
      languages: buildLanguageAlternates("/ai-tools/emoji-translator"),
    },
    openGraph: {
      title: metadata.title,
      description: metadata.description,
...
export default async function EmojiTranslatorPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const currentUrl = `${homeUrl}${locale === "en" ? "" : `/${locale}`}/ai-tools/emoji-translator`;
    ...
    "@graph": [
      {
        ...
        itemListElement: [
          ...
          },
          {
            "@type": "ListItem",
            position: 3,
            name: section.ui?.breadcrumb_current ?? "Emoji Translator",
            item: currentUrl,
          },
        ],
      },
      {
        "@type": "WebApplication",
        name: section.ui?.title ?? "Emoji Translator",
  ...
  return (
    <>
      ...
      />
      <EmojiTranslator section={section} />
