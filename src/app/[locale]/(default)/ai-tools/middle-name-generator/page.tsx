import MiddleNameGenerator from "@/components/blocks/middle-name-generator";
...
export async function generateMetadata({
  ...
  const messages = await import(`@/i18n/pages/middle-name-generator/${locale}.json`);
  const section = messages.default.middle_name_generator;
  const metadata = section.metadata;

  const canonicalUrl =
    locale === "en"
      ? `${process.env.NEXT_PUBLIC_WEB_URL}/ai-tools/middle-name-generator`
      : `${process.env.NEXT_PUBLIC_WEB_URL}/${locale}/ai-tools/middle-name-generator`;
  ...
  return {
    ...
    alternates: {
      ...
      languages: buildLanguageAlternates("/ai-tools/middle-name-generator"),
    },
    openGraph: {
      title: metadata.title,
      description: metadata.description,
...
export default async function MiddleNameGeneratorPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const currentUrl = `${homeUrl}${locale === "en" ? "" : `/${locale}`}/ai-tools/middle-name-generator`;
    ...
    "@graph": [
      {
        ...
        itemListElement: [
          ...
          },
          {
            "@type": "ListItem",
            position: 3,
            name: section.ui?.breadcrumb_current ?? "Middle Name Generator",
            item: currentUrl,
          },
        ],
      },
      {
        "@type": "WebApplication",
        name: section.ui?.title ?? "Middle Name Generator",
  ...
  return (
    <>
      ...
      />
      <MiddleNameGenerator section={section} />
      ...
      <RelatedTools
        currentSlug="middle-name-generator"
        relatedSlugs={[
          "pen-name-generator",
          "backstory-generator",
          "dialogue-generator",
