import GangNameGenerator from "@/components/blocks/gang-name-generator";
...
export async function generateMetadata({
  ...
  const messages = await import(`@/i18n/pages/gang-name-generator/${locale}.json`);
  const section = messages.default.gang_name_generator;
  const metadata = section.metadata;

  const canonicalUrl =
    locale === "en"
      ? `${process.env.NEXT_PUBLIC_WEB_URL}/ai-tools/gang-name-generator`
      : `${process.env.NEXT_PUBLIC_WEB_URL}/${locale}/ai-tools/gang-name-generator`;
  ...
  return {
    ...
    alternates: {
      ...
      languages: buildLanguageAlternates("/ai-tools/gang-name-generator"),
    },
    openGraph: {
      title: metadata.title,
      description: metadata.description,
...
export default async function GangNameGeneratorPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const currentUrl = `${homeUrl}${locale === "en" ? "" : `/${locale}`}/ai-tools/gang-name-generator`;
    ...
    "@graph": [
      {
        ...
        itemListElement: [
          ...
          },
          {
            "@type": "ListItem",
            position: 3,
            name: section.ui?.breadcrumb_current ?? "Gang Name Generator",
            item: currentUrl,
          },
        ],
      },
      {
        "@type": "WebApplication",
        name: section.ui?.title ?? "Gang Name Generator",
  ...
  return (
    <>
      ...
      />
      <GangNameGenerator section={section} />
      ...
      <RelatedTools
        currentSlug="gang-name-generator"
        relatedSlugs={[
          "plot-generator",
          "backstory-generator",
