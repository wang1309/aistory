import ElfNameGenerator from "@/components/blocks/elf-name-generator";
...
export async function generateMetadata({
  ...
  const messages = await import(`@/i18n/pages/elf-name-generator/${locale}.json`);
  const section = messages.default.elf_name_generator;
  ...
  return {
    ...
    alternates: {
      ...
      languages: buildLanguageAlternates("/ai-tools/elf-name-generator"),
    },
    openGraph: {
      title: metadata.title,
      description: metadata.description,
...
export default async function ElfNameGeneratorPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const currentUrl = `${homeUrl}${locale === "en" ? "" : `/${locale}`}/ai-tools/elf-name-generator`;
    ...
    "@graph": [
      {
        ...
        itemListElement: [
          ...
          },
          {
            "@type": "ListItem",
            position: 3,
            name: section.ui?.breadcrumb_current ?? "Elf Name Generator",
            item: currentUrl,
          },
        ],
      },
      {
        "@type": "WebApplication",
        name: section.ui?.title ?? "Elf Name Generator",
  ...
  return (
    <>
      ...
      />
      <ElfNameGenerator section={section} />
      ...
      <RelatedTools
        currentSlug="elf-name-generator"
        relatedSlugs={[
          "fantasy-generator",
