...
export async function generateMetadata({
  ...
  if (locale !== "en") {
    canonicalUrl = `${process.env.NEXT_PUBLIC_WEB_URL}/${locale}/ai-write-tool`;
  }

  return {
    title: `${t("ai_tools.section_title_hub")} - AI Write Tool`,
    description: t("ai_tools.section_description_hub"),
    alternates: {
      canonical: canonicalUrl,
      languages: buildLanguageAlternates("/ai-write-tool"),
    },
  };
}

export default async function AiWriteToolPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const structuredData = {
    ...
    "@type": "ItemList",
    name: `${t("ai_tools.section_title_hub")} - AI Write Tool`,
    description: t("ai_tools.section_description_hub"),
