...
import {
  generatePenNames,
  validatePenNameRequest,
} from "@/app/api/pen-name/_lib";
import type { PenNameRouteRequest } from "@/types/pen-name-generator";
import {
...
export async function POST(req: Request) {
  try {
    const body = (await req.json()) as PenNameRouteRequest;
    const token = typeof body.turnstileToken === "string" ? body.turnstileToken : "";
    if (!token) return respErr("Verification failed. Try again.");

    const validated = validatePenNameRequest(body);
    ...
    try {
      const result = await generatePenNames(validated.value);
      ...
      return withCreativeVisitorCookie(
        respErr("Unable to generate pen names. Please try again."),
        quotaGate
      );
    }
  } catch (error) {
    // Do not log request fields, tokens, prompt, upstream body, or AI output.
    console.error("pen name request failed", error);
    return respErr("Unable to generate pen names. Please try again.");
  }
}
