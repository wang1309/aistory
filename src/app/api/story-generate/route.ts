...
async function verifyTurnstileToken(token: string): Promise<boolean> {
  ...
  try {
    ...
    if (cached) {
      console.log("Using Turnstile KV cache (Story Gen)");
      return true;
    }
  } catch (e) {
    console.log("Turnstile KV cache check failed (Story Gen)", e);
  }
  ...
  try {
    ...
    if (success) {
      try {
        ...
        console.log("Turnstile KV cache write failed (Story Gen)", e);
      }
    }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return withCreativeVisitorCookie(
        respErr(`API Error: ${response.status} - ${errorText}`),
        quotaGate
      );
    }
