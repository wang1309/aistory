...
import {
  generateMiddleNames,
  validateMiddleNameRequest,
} from "@/app/api/middle-name/_lib";
import type { MiddleNameRouteRequest } from "@/types/middle-name-generator";
import {
...
export async function POST(req: Request) {
  try {
    const body = (await req.json()) as MiddleNameRouteRequest;
    const token = typeof body.turnstileToken === "string" ? body.turnstileToken : "";
    if (!token) return respErr("Verification failed. Try again.");

    const validated = validateMiddleNameRequest(body);
    ...
    try {
      const result = await generateMiddleNames(validated.value);
      ...
      return withCreativeVisitorCookie(
        respErr("Unable to generate middle names. Please try again."),
        quotaGate
      );
    }
  } catch (error) {
    // Do not log request fields, tokens, prompt, upstream body, or AI output.
    console.error("middle name generation failed", error);
    return respErr("Unable to generate middle names. Please try again.");
  }
}
