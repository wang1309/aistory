  ...
  return (
    <div className="space-y-2">
      ...
      <textarea
        ...
        rows={6}
        className="w-full resize-none rounded-lg border border-border/40 px-3 py-2 text-xs outline-none placeholder:text-muted-foreground/50 focus:border-primary/60"
      />
      <div className="flex items-center gap-2">
        <Button
          size="sm"
          className="h-7 rounded-lg bg-primary px-3 text-xs text-white hover:bg-primary/90"
          disabled={isSaving}
        ...
        <div>
          ...
          {isLoading && (
            <div className="flex items-center justify-center gap-2 py-8">
              ...
              <span className="inline-block size-1.5 animate-pulse rounded-full bg-primary/50 [animation-delay:0.2s]" />
              <span className="inline-block size-1.5 animate-pulse rounded-full bg-primary/50 [animation-delay:0.4s]" />
            </div>
          )}
          ...
          {!isLoading && (
            <>
              {/* Create new form */}
              {isCreating && (
                <div className="mb-3 rounded-xl border border-primary/50 bg-primary/30 p-3 dark:border-primary/30 dark:bg-orange-900/10">
                  <EditForm
                    initName=""
                    initSample=""
                    copy={copy}
                    ...
                    <div>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          {fp.is_active && (
                            <span className="rounded-full bg-primary/10 px-2 py-0.5 text-[10px] font-medium text-primary dark:bg-orange-900/30 dark:text-primary/15">
                              {copy.active}
                            </span>
                          )}
                        </div>
                      ...
                      <div className="mt-2 flex items-center gap-2">
                        {fp.is_active ? (
                          ...
                          <button
                            ...
                            disabled={togglingUuid === fp.uuid}
                            className="rounded-md px-2 py-1 text-[11px] font-medium text-primary transition hover:bg-primary/5 disabled:opacity-60 dark:text-primary/25 dark:hover:bg-orange-900/20"
                          >
                            {togglingUuid === fp.uuid ? (
                              <span className="inline-flex items-center gap-1">
                                <span className="inline-block size-1.5 animate-pulse rounded-full bg-primary" />
                                <span className="inline-block size-1.5 animate-pulse rounded-full bg-primary [animation-delay:0.15s] dark:bg-primary/25" />
                                <span className="inline-block size-1.5 animate-pulse rounded-full bg-primary [animation-delay:0.3s] dark:bg-primary/25" />
                              </span>
                            ) : copy.activate}
                          </button>
                        )}
              ...
              {!isCreating && (
                <button
                  ...
                  className="w-full rounded-lg border border-dashed border-border/60 py-2 text-xs text-muted-foreground transition hover:border-primary/60 hover:bg-primary/50 hover:text-primary dark:hover:bg-orange-900/10"
                >
                  {copy.newStyle}
                </button>
              )}
